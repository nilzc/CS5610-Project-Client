...
const UpcomingMoviesScreen = () => {
    ...
    const findUpcomingMovies = useCallback(
        () => {
            movieServices.findUpcomingMovies(currPage).then(ms => setMovies(ms)).catch(errorServices.alertError);
        }, [currPage]
    )
    useEffect(findUpcomingMovies, [findUpcomingMovies]);
    return (
        <div className={"m-4"}>
