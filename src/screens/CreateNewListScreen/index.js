            ...
            <ul className="list-group">
                ...
                movieSelected.map((litem, t) => {
                    ...
                    <li className="mt-2 list-group-item d-flex align-items-start" key={t}>
                        ...
                        <div className="ms-4 fw-bold fs-5" style={{float: 'right', color: 'red'}}><span onClick={() => deleteMSOnClickHandler(litem)}><i className="fas fa-times"></i></span></div>
                      </li>
           
                    );
                })}
                <button className="btn btn-primary mb-3 mt-3 rounded-pill fs-5" onClick={submitHandler}>Submit</button>
            </ul>
        </div>
    )
};
